...
static long ssv_sdiobridge_ioctl_setFuncfocus(struct ssv_sdiobridge_glue *glue,unsigned int cmd, struct ssv_sdiobridge_cmd *pcmd_data,struct ssv_sdiobridge_cmd *pucmd_data,bool isCompat)
{
    ...
    else
    {
        if ( isCompat )
        {
            copy_from_user(&glue->funcFocus,(int __user *)compat_ptr((unsigned long)pcmd_data->in_data),sizeof(glue->funcFocus));
        }
        else
        {
            copy_from_user(&glue->funcFocus,(int __user *)pcmd_data->in_data,sizeof(glue->funcFocus));
        }
    }
...
static long ssv_sdiobridge_ioctl_getBlockSize(struct ssv_sdiobridge_glue *glue,unsigned int cmd, struct ssv_sdiobridge_cmd *pcmd_data,struct ssv_sdiobridge_cmd *pucmd_data,bool isCompat)
{
    ...
    else
    {
        if ( isCompat )
        {
            copy_to_user((int __user *)compat_ptr((unsigned long)pucmd_data->out_data),&glue->blockSize,sizeof(glue->blockSize));
        }
        else
        {
            copy_to_user((int __user *)pucmd_data->out_data,&glue->blockSize,sizeof(glue->blockSize));
        }
    }
...
static long ssv_sdiobridge_ioctl_setBlockSize(struct ssv_sdiobridge_glue *glue,unsigned int cmd, struct ssv_sdiobridge_cmd *pcmd_data,struct ssv_sdiobridge_cmd *pucmd_data,bool isCompat)
{
    ...
    else
    {
        if ( isCompat )
        {
            copy_from_user(&glue->blockSize,(int __user *)compat_ptr((unsigned long)pucmd_data->in_data),sizeof(glue->blockSize));
        }
        else
        {
            copy_from_user(&glue->blockSize,(int __user *)pucmd_data->in_data,sizeof(glue->blockSize));
        }
...
static long ssv_sdiobridge_ioctl_readByte(struct ssv_sdiobridge_glue *glue,unsigned int cmd, struct ssv_sdiobridge_cmd *pcmd_data,struct ssv_sdiobridge_cmd *pucmd_data,bool isCompat)
{
    ...
    else
    {
        ...
        if ( isCompat )
        {
            copy_from_user(&address,(int __user *)compat_ptr((unsigned long)pucmd_data->in_data),sizeof(address));
        }
        else
        {
            copy_from_user(&address,(int __user *)pucmd_data->in_data,sizeof(address));
        }
        ...
        if ( !ret )
        {
            if ( isCompat )
            {
                copy_to_user((void *)compat_ptr((unsigned long)pucmd_data->out_data),&out_data,sizeof(out_data));
            }
            else
            {
                copy_to_user((int __user *)pucmd_data->out_data,&out_data,sizeof(out_data));
...
static long ssv_sdiobridge_ioctl_writeByte(struct ssv_sdiobridge_glue *glue,unsigned int cmd, struct ssv_sdiobridge_cmd *pcmd_data,struct ssv_sdiobridge_cmd *pucmd_data,bool isCompat)
{
    ...
    else
    {
        ...
        if ( isCompat )
        {
            copy_from_user(tmp,(int __user *)compat_ptr((unsigned long)pucmd_data->in_data),sizeof(tmp));
        }
        else
        {
            copy_from_user(tmp,(int __user *)pucmd_data->in_data,sizeof(tmp));
        }
        address = *((u32 *)tmp);
        data = tmp[4];
...
static long ssv_sdiobridge_ioctl_getMultiByteIOPort(struct ssv_sdiobridge_glue *glue,unsigned int cmd, struct ssv_sdiobridge_cmd *pcmd_data,struct ssv_sdiobridge_cmd *pucmd_data,bool isCompat)
{
    ...
    else
    {
        if ( isCompat )
        {
            copy_to_user((int __user *)compat_ptr((unsigned long)pucmd_data->out_data),&glue->dataIOPort,sizeof(glue->dataIOPort));
        }
        else
        {
            copy_to_user((int __user *)pucmd_data->out_data,&glue->dataIOPort,sizeof(glue->dataIOPort));
        }
    }
...
static long ssv_sdiobridge_ioctl_setMultiByteIOPort(struct ssv_sdiobridge_glue *glue,unsigned int cmd, struct ssv_sdiobridge_cmd *pcmd_data,struct ssv_sdiobridge_cmd *pucmd_data,bool isCompat)
{
    ...
    else
    {
        if ( isCompat )
        {
            copy_from_user(&glue->dataIOPort,(int __user *)compat_ptr((unsigned long)pucmd_data->in_data),sizeof(glue->dataIOPort));
        }
        else
        {
            copy_from_user(&glue->dataIOPort,(int __user *)pucmd_data->in_data,sizeof(glue->dataIOPort));
        }
    }
...
static long ssv_sdiobridge_ioctl_readMultiByte(struct ssv_sdiobridge_glue *glue,unsigned int cmd, struct ssv_sdiobridge_cmd *pcmd_data,struct ssv_sdiobridge_cmd *pucmd_data,bool isCompat)
{
    ...
    if ( !ret )
    {
        if ( isCompat )
        {
            copy_to_user((int __user *)compat_ptr((unsigned long)pucmd_data->out_data),tmpdata,pcmd_data->out_data_len);
        }
        else
        {
            copy_to_user((int __user *)pucmd_data->out_data,tmpdata,pcmd_data->out_data_len);
        }
    }
    else
    {
...
static long ssv_sdiobridge_ioctl_writeMultiByte(struct ssv_sdiobridge_glue *glue,unsigned int cmd, struct ssv_sdiobridge_cmd *pcmd_data,struct ssv_sdiobridge_cmd *pucmd_data,bool isCompat)
{
    ...
    if ( isCompat )
    {
        copy_from_user(tmpdata,(int __user *)compat_ptr((unsigned long)pucmd_data->in_data),pcmd_data->in_data_len);
    }
    else
    {
        copy_from_user(tmpdata,(int __user *)pucmd_data->in_data,pcmd_data->in_data_len);
    }
...
static long ssv_sdiobridge_ioctl_getMultiByteRegIOPort(struct ssv_sdiobridge_glue *glue,unsigned int cmd, struct ssv_sdiobridge_cmd *pcmd_data,struct ssv_sdiobridge_cmd *pucmd_data,bool isCompat)
{
    ...
    else
    {
        if ( isCompat )
        {
            copy_to_user((int __user *)compat_ptr((unsigned long)pucmd_data->out_data),&glue->regIOPort,sizeof(glue->regIOPort));
        }
        else
        {
            copy_to_user((int __user *)pucmd_data->out_data,&glue->regIOPort,sizeof(glue->regIOPort));
        }
    }
...
static long ssv_sdiobridge_ioctl_setMultiByteRegIOPort(struct ssv_sdiobridge_glue *glue,unsigned int cmd, struct ssv_sdiobridge_cmd *pcmd_data,struct ssv_sdiobridge_cmd *pucmd_data,bool isCompat)
{
    ...
    else
    {
        if ( isCompat )
        {
            copy_from_user(&glue->regIOPort,(int __user *)compat_ptr((unsigned long)pucmd_data->in_data),sizeof(glue->regIOPort));
        }
        else
        {
            copy_from_user(&glue->regIOPort,(int __user *)pucmd_data->in_data,sizeof(glue->regIOPort));
        }
    }
...
static long ssv_sdiobridge_ioctl_readReg(struct ssv_sdiobridge_glue *glue,unsigned int cmd, struct ssv_sdiobridge_cmd *pcmd_data,struct ssv_sdiobridge_cmd *pucmd_data,bool isCompat)
{
    ...
    else
    {
        ...
        if ( isCompat )
        {
            copy_from_user(tmpdata,(int __user *)compat_ptr((unsigned long)pucmd_data->in_data),sizeof(tmpdata));
        }
        else
        {
            copy_from_user(tmpdata,(int __user *)pucmd_data->in_data,sizeof(tmpdata));
        }
        ...
        if ( !ret )
        {
            if ( isCompat )
            {
                copy_to_user((int __user *)compat_ptr((unsigned long)pucmd_data->out_data),tmpdata,sizeof(tmpdata));
            }
            else
            {
                copy_to_user((int __user *)pucmd_data->out_data,tmpdata,sizeof(tmpdata));
            }
        }
        else
        {
...
static long ssv_sdiobridge_ioctl_writeReg(struct ssv_sdiobridge_glue *glue,unsigned int cmd, struct ssv_sdiobridge_cmd *pcmd_data,struct ssv_sdiobridge_cmd *pucmd_data,bool isCompat)
{
    ...
    else
    {
        ...
        dev_err(glue->dev,"%s: write reg [%02X][%02X][%02X][%02X] [%02X][%02X][%02X][%02X]\n",__FUNCTION__,tmpdata[0],tmpdata[1],tmpdata[2],tmpdata[3],tmpdata[4],tmpdata[5],tmpdata[6],tmpdata[7]);
        ...
        loff_t *offset)
{
    ...
    copy_to_user((int __user *)buffer,bf->rxdata,copylength);
        ...
        loff_t *off)
{
    ...
    copy_from_user(tmpdata,(int __user *)buff,len);
        ...
        struct ssv_sdiobridge_glue *glue)
{
    ...
    glue->regIOPort = glue->regIOPort | (sdio_readb(func, REG_REG_IO_PORT_0, &err_ret) << ( 8*1 ));
    glue->regIOPort = glue->regIOPort | (sdio_readb(func, REG_REG_IO_PORT_0, &err_ret) << ( 8*2 ));
        ...
        const struct sdio_device_id *id)
{
    ...
    if (ssv_devicetype != 1) {
        pr_err("Not using SSV6200 SDIO bridge driver.\n");
        return -ENODEV;
    }

    dev_err(&func->dev, "ssv_sdiobridge_probe func->num:%d",func->num);
...
#ifdef CONFIG_SSV_SDIO_BRIDGE_DEBUGFS
    ssv_sdiobridge_init_debug(glue);
#endif
...
#ifdef CONFIG_SSV_SDIO_BRIDGE_DEBUGFS
        ssv_sdiobridge_deinit_debug(glue);
#endif
        kfree(glue);
        glue = NULL;
    }
