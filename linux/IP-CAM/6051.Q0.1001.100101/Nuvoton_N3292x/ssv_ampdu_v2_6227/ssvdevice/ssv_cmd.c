...
static int ssv_cmd_help(int argc, char *argv[])
{
    ...
    if (argc==2 && !strcmp(argv[1], "mib") ){
        sprintf(tmpbf, "mib [reset|list|rx]\n\n");
        strcat(ssv6xxx_result_buf, tmpbf);

    }
    else if (argc==2 && !strcmp(argv[1], "reg") ){        
        sprintf(tmpbf, "reg [r|w] [address] [value|word-count]\n\n");
        strcat(ssv6xxx_result_buf, tmpbf);

    }
    else if (argc==2 && !strcmp(argv[1], "irq") ){        
        sprintf(tmpbf, "irq [set|get] [mask|enable|disable|status]\n\n");
        strcat(ssv6xxx_result_buf, tmpbf);

    }
    else if (argc==2 && !strcmp(argv[1], "hci") ){        
        sprintf(tmpbf, "hci [txq|rxq] [show]\nhci [isr_time] [start|stop|show]\n\n");
        strcat(ssv6xxx_result_buf, tmpbf);    
    
    }
    else if (argc==2 && !strcmp(argv[1], "mac") ){        
           sprintf(tmpbf, "mac [security|wsid|rxq] [show]\nmac [set|get] [rate] [auto|idx]\nmac [rx|tx] [eable|disable]\n\n");
           strcat(ssv6xxx_result_buf, tmpbf);    
...
static struct ssv6xxx_cfg_cmd_table cfg_cmds[] = {
    { "drv_type",           (void *)&ssv_cfg.drv_type,     0,   __string2u8         },
    { "hw_mac",             (void *)&ssv_cfg.maddr[0],     0,   __string2mac        },
    { "def_chan",           (void *)&ssv_cfg.def_chan,     0,   __string2u8         },
    { "txpb_offset",        (void *)&ssv_cfg.txpb_offset,  0,   __string2u8         }, 
    { "rxpb_ofset",         (void *)&ssv_cfg.rxpb_offset,  0,   __string2u8         },
    ...
    { "hw_cap_dfs",         (void *)&ssv_cfg.hw_caps,      4,   __string2flag32     },
    { "hw_cap_security",    (void *)&ssv_cfg.hw_caps,      5,   __string2flag32     },
    { "hw_cap_rc",          (void *)&ssv_cfg.hw_caps,      6,   __string2flag32     },
    { "hw_cap_sgi_20",      (void *)&ssv_cfg.hw_caps,      7,   __string2flag32     },
    { "hw_cap_sgi_40",      (void *)&ssv_cfg.hw_caps,      8,   __string2flag32     },
    { "hw_cap_ldpc",        (void *)&ssv_cfg.hw_caps,      9 ,  __string2flag32     },
    { "hw_cap_stbc",        (void *)&ssv_cfg.hw_caps,      10,  __string2flag32     },
    { "hw_cap_b",           (void *)&ssv_cfg.hw_caps,      11,  __string2flag32     },
    { "hw_cap_g",           (void *)&ssv_cfg.hw_caps,      12,  __string2flag32     },
    { "hw_cap_ap",          (void *)&ssv_cfg.hw_caps,      13,  __string2flag32     },
    { "hw_cap_p2p",         (void *)&ssv_cfg.hw_caps,      14,  __string2flag32     },
    { "hw_cap_dup_detect",  (void *)&ssv_cfg.hw_caps,      15,  __string2flag32     },
    { "hw_cap_mac_filer",   (void *)&ssv_cfg.hw_caps,      16,  __string2flag32     },
    { "hw_cap_l34_offload", (void *)&ssv_cfg.hw_caps,      17,  __string2flag32     },
    { "hw_cap_ampdu_rx",    (void *)&ssv_cfg.hw_caps,      18,  __string2flag32     },
    { "hw_cap_ampdu_tx",    (void *)&ssv_cfg.hw_caps,      19,  __string2flag32     },
    { NULL, NULL, 0, NULL },
};
...
static int ssv_cmd_cfg(int argc, char *argv[])
{
    ...
    if (argc==2 && strcmp(argv[1], "show")==0) {
        ...
        sprintf(temp_buf, "    drv_type = %d\n", ssv_cfg.drv_type);
        strcat(ssv6xxx_result_buf, temp_buf);
        sprintf(temp_buf, "    hw_caps = 0x%08x\n", ssv_cfg.hw_caps);
        strcat(ssv6xxx_result_buf, temp_buf);
        sprintf(temp_buf, "    def_chan = %d\n", ssv_cfg.def_chan);
        strcat(ssv6xxx_result_buf, temp_buf);
        sprintf(temp_buf, "    txpb_offset = %d\n", ssv_cfg.txpb_offset);
        strcat(ssv6xxx_result_buf, temp_buf);
        sprintf(temp_buf, "    rxpb_offset = %d\n", ssv_cfg.rxpb_offset);
        strcat(ssv6xxx_result_buf, temp_buf);
        sprintf(temp_buf, "    sta-mac = %02x:%02x:%02x:%02x:%02x:%02x",
            ssv_cfg.maddr[0], ssv_cfg.maddr[1], ssv_cfg.maddr[2], 
            ssv_cfg.maddr[3], ssv_cfg.maddr[4], ssv_cfg.maddr[5]);
        strcat(ssv6xxx_result_buf, temp_buf);
        strcat(ssv6xxx_result_buf, "\n");
        return 0;
    }
...
static int ssv_cmd_sta(int argc, char *argv[])
{
	...
	if (argc >= 2 && strcmp(argv[1], "show")==0) {
		...
		sta_info = ssv_dbg_sc->sta_info;

        sprintf(tmpbf, "  >>>> bcast queue len[%d]\n", ssv_dbg_sc->bcast_txq.cur_qsize);
		strcat(ssv6xxx_result_buf, tmpbf);
		
		for(i=0; i<SSV_NUM_STA; i++,sta_info++){
		    if ((sta_info->s_flags & STA_FLAG_VALID)!=0) {
				sprintf(tmpbf, "  >>>> Station[%d]:\n     Address[%02x][%02x][%02x][%02x][%02x][%02x]\n     WISD[%d] AID[%d] Sleep[%d]\n",
								i, sta_info->sta->addr[0], sta_info->sta->addr[1],sta_info->sta->addr[2],
...
{    
    ...
    if (strcmp(argv[1], "phy-reg") == 0) {
        ...
        for(s=0; s<ssv_dbg_phy_len; s++, raw++) {
            if(SSV_REG_READ1(ssv6xxx_debug_ifops, raw->addr, &regval));
...
static int ssv_cmd_hci(int argc, char *argv[])
{
    ...
    {  
        for(s=0; s<SSV_HW_TXQ_NUM; s++) {
            int ac = ssv_dbg_sc->tx.ac_txqid[s];
            txq = &ssv_dbg_ctrl_hci->hw_txq[s];                       
            sprintf(temp_str, ">> txq[%d](%s): cur_qsize=%d\n", txq->txq_no,
                ((ssv_dbg_sc->sc_flags&SC_OP_OFFCHAN)? "off channel": "on channel"),
                skb_queue_len(&txq->qhead));
            strcat(ssv6xxx_result_buf, temp_str);
            sprintf(temp_str, "            max_qsize=%d, pause=%d, resume_thres=%d flow_control[%d]\n", 
                txq->max_qsize, txq->paused, txq->resum_thres, !!(ssv_dbg_sc->tx.flow_ctrl_status&(1<<ac)));
            strcat(ssv6xxx_result_buf, temp_str);
    ...
    if (argc==3 && !strcmp(argv[1], "isr_time") && !strcmp(argv[2], "start") ) {
    ...
    if (argc==3 && !strcmp(argv[1], "isr_time") && !strcmp(argv[2], "stop") ) {                
    ...
    if (argc==3 && !strcmp(argv[1], "isr_time") && !strcmp(argv[2], "show") ) {
        print_isr_info();
        return 0;
    }

#ifdef CONFIG_IRQ_DEBUG_COUNT
	if (argc==3 && !strcmp(argv[1], "isr_debug") && !strcmp(argv[2], "reset") ) {
    ...
    if (argc==3 && !strcmp(argv[1], "isr_debug") && !strcmp(argv[2], "show") ) {
		print_irq_count();
        return 0;
    }
	if (argc==3 && !strcmp(argv[1], "isr_debug") && !strcmp(argv[2], "stop") ) {
		ssv_dbg_ctrl_hci->irq_enable= 0;
		strcat(ssv6xxx_result_buf, "irq debug stop\n");
        return 0;
    }
	if (argc==3 && !strcmp(argv[1], "isr_debug") && !strcmp(argv[2], "start") ) {
...
static int ssv_cmd_iqk (int argc, char *argv[]) {
    ...
    if ((argc == 3) && (strcmp(argv[1], "cfg-pa") == 0)) {
    ...
    else if ((argc == 3) && (strcmp(argv[1], "tk-chch") == 0)) {
        // tk-chch
        cmd_iqk_cfg.cmd_sel = SSV6XXX_IQK_CMD_TK_CHCH;
        cmd_iqk_cfg.argv    = simple_strtoul(argv[2], &endp, 0);

        sprintf(temp_str,"## do tk-chch\n");
        strcat(ssv6xxx_result_buf, temp_str);
    }
    else if ((argc == 2) && (strcmp(argv[1], "tk-rxcnt-reset") == 0)) {
        // tk-rxcnt
        cmd_iqk_cfg.cmd_sel = SSV6XXX_IQK_CMD_TK_RXCNT;
        cmd_iqk_cfg.argv    = 0;

        sprintf(temp_str,"## do tk-rxcnt-reset\n");
        strcat(ssv6xxx_result_buf, temp_str);
    }
        ...
        strcat(ssv6xxx_result_buf, temp_str);
        sprintf(temp_str,"## cmd: init-cali/rtbl-load/rtbl-load-def/rtbl-reset/rtbl-set/rtbl-export/tk-evm/tk-tone/tk-chch\n");
        strcat(ssv6xxx_result_buf, temp_str);
...
static int ssv_cmd_version (int argc, char *argv[]) {
    ...
    *((u32 *)&chip_id[12]) = (u32)LONGSWAP(regval);
    ...
    sprintf(temp_str,"# modified files:\n");
    strcat(ssv6xxx_result_buf, temp_str);

    for(loop_i = 0; loop_i < sizeof(files_modified)/(sizeof(char)*FILE_NAME_SIZE); loop_i++) {
        sprintf(temp_str,"## %s\n", files_modified[loop_i]);
        strcat(ssv6xxx_result_buf, temp_str);
    }

    sprintf(temp_str,"# otherversion files:\n");
    strcat(ssv6xxx_result_buf, temp_str);

    for(loop_i = 0; loop_i < sizeof(files_otherversion)/(sizeof(char)*80); loop_i++) {
        sprintf(temp_str,"## %s\n", files_otherversion[loop_i]);
        strcat(ssv6xxx_result_buf, temp_str);
    }
...
static int ssv_cmd_loadfw (int argc, char *argv[]) {
    ssv6xxx_debug_ifops->ifops->load_fw(ssv6xxx_debug_ifops->dev);
    return 0;
}
