/*
 *	linux/drivers/net/wireless/ssv6200/cabriofmac/cabrio_spi.c
 *
 *	Driver for South Silicon Valley Co., Lmt. Cabrio WLAN cards.
 *
 *	Copyright 2013 South Silicon Valley Co., Lmt.
 *
 *	Authors:
 *
...
#include <linux/spi/cabrio_spi.h>
...
struct if_spi_card {
	...
	struct cabrio_spi_platform_data *pdata;
	...
	/* A buffer of incoming packets from cabrio core.
...
static const struct cabrio_fw_table fw_table[] = {
	{ MODEL_8385, "cabrio/gspi8385_helper.bin", "cabrio/gspi8385.bin" },
	{ MODEL_8385, "cabrio/gspi8385_hlp.bin", "cabrio/gspi8385.bin" },
	{ MODEL_8686, "cabrio/gspi8686_v9_helper.bin", "cabrio/gspi8686_v9.bin" },
	{ MODEL_8686, "cabrio/gspi8686_hlp.bin", "cabrio/gspi8686.bin" },
	{ MODEL_8688, "cabrio/gspi8688_helper.bin", "cabrio/gspi8688.bin" },
	{ 0, NULL, NULL }
};
MODULE_FIRMWARE("cabrio/gspi8385_helper.bin");
MODULE_FIRMWARE("cabrio/gspi8385_hlp.bin");
MODULE_FIRMWARE("cabrio/gspi8385.bin");
MODULE_FIRMWARE("cabrio/gspi8686_v9_helper.bin");
MODULE_FIRMWARE("cabrio/gspi8686_v9.bin");
MODULE_FIRMWARE("cabrio/gspi8686_hlp.bin");
MODULE_FIRMWARE("cabrio/gspi8686.bin");
MODULE_FIRMWARE("cabrio/gspi8688_helper.bin");
MODULE_FIRMWARE("cabrio/gspi8688.bin");
...
static int spu_init(struct if_spi_card *card, int use_dummy_writes)
{
	...
	cabrio_deb_spi("Initialized SPU unit. "
					...
					const struct firmware *firmware)
{
	...
	cabrio_deb_enter(CABRIO_DBG_SPI);
...
out:
	...
	cabrio_deb_leave_args(CABRIO_DBG_SPI, "err %d", err);
	return err;
}
...
static int if_spi_c2h_cmd(struct if_spi_card *card)
{
	...
	cabrio_dbg_enter(CABRIO_DBG_SPI);
...
out:
	...
	cabrio_dbg_leave(CABRIO_DBG_SPI);
	return err;
}
...
static int if_spi_c2h_data(struct if_spi_card *card)
{
	...
	if (!len) {
		...
		err = -EINVAL;
		goto out;
	} else if (len > CABRIO_ETH_RX_PACKET_BUFFER_SIZE) {
		netdev_err(priv->dev,
			   "%s: error: card has %d bytes of data, but our maximum skb size is %zu\n",
			   __func__, len, CABRIO_ETH_RX_PACKET_BUFFER_SIZE);
		err = -EINVAL;
		goto out;
	}

	/* TODO: should we allocate a smaller skb if we have less data? */
	skb = dev_alloc_skb(CABRIO_ETH_RX_PACKET_BUFFER_SIZE);
	if (!skb) {
		err = -ENOBUFS;
		goto out;
	}
	...
	/* pass the SKB to cabrio */
	err = cabrio_process_rxed_packet(card->priv, skb);
	if (err)
		goto free_skb;
...
static void if_spi_e2h(struct if_spi_card *card)
{
	...
	cabrio_queue_event(priv, cause & 0xff);
out:
	if (err)
		netdev_err(priv->dev, "%s: error %d\n", __func__, err);
}
...
static void if_spi_host_to_card_worker(struct work_struct *work)
{
	...
	if (hiStatus & IF_SPI_HIST_CMD_DOWNLOAD_RDY ||
	   (card->priv->psstate != PS_STATE_FULL_POWER &&
		...
		if (packet)
			if_spi_h2c(card, packet, CABRIO_MS_CMD);
	}
	if (hiStatus & IF_SPI_HIST_TX_DOWNLOAD_RDY) {
		...
		if (packet)
			if_spi_h2c(card, packet, CABRIO_MS_DAT);
	}
 ...
 * We can't sleep here.
 */
static int if_spi_host_to_card(struct cabrio_private *priv,
				u8 type, u8 *buf, u16 nb)
{
	...
	cabrio_dbg_enter_args(CABRIO_DBG_SPI, "type %d, bytes %d", type, nb);
...
out:
	cabrio_dbg_leave_args(CABRIO_DBG_SPI, "err=%d", err);
	return err;
}
...
static int if_spi_init_card(struct if_spi_card *card)
{
	...
	if (scratch == SUCCESSFUL_FW_DOWNLOAD_MAGIC)
		cabrio_dbg_spi("Firmware is already loaded for "
		...
		err = cabrio_get_firmware(&card->spi->dev, NULL, NULL,
		...
		cabrio_dbg_spi("Initializing FW for Marvell WLAN 802.11 adapter "
		...
		if (err)
			goto out;
		cabrio_dbg_spi("loaded FW for Marvell WLAN 802.11 adapter\n");
	}
...
out:
	...
	cabrio_dbg_leave_args(CABRIO_DBG_SPI, "err %d\n", err);
...
static int __devinit if_spi_probe(struct spi_device *spi)
{
	...
	struct cabrio_spi_platform_data *pdata = spi->dev.platform_data;
	int err = 0;
	...
	/*
	 * Register our card with cabrio.
	 * This will call alloc_etherdev.
	 */
	priv = cabrio_add_card(card, &spi->dev);
	if (!priv) {
		err = -ENOMEM;
		goto free_card;
	}
	...
	card->workqueue = create_workqueue("cabrio_spi");
	INIT_WORK(&card->packet_work, if_spi_host_to_card_worker);
	INIT_WORK(&card->resume_work, if_spi_resume_worker);

	err = request_irq(spi->irq, if_spi_host_interrupt,
			IRQF_TRIGGER_FALLING, "cabrio_spi", card);
	if (err) {
	 ...
	 * getting interrupts...
	 */
	err = cabrio_start_card(priv);
	if (err)
		goto release_irq;

	cabrio_dbg_spi("Finished initializing WLAN module.\n");
...
terminate_workqueue:
	...
	cabrio_remove_card(priv); /* will call free_netdev */
free_card:
...
static int __devexit cabrio_spi_remove(struct spi_device *spi)
{
	struct if_spi_card *card = spi_get_drvdata(spi);
	struct cabrio_private *priv = card->priv;

	cabrio_dbg_spi("cabrio_spi_remove\n");
	cabrio_dbg_enter(CABRIO_DBG_SPI);

	cancel_work_sync(&card->resume_work);

	cabrio_stop_card(priv);
	cabrio_remove_card(priv); /* will call free_netdev */
...
static struct spi_driver cabrio_spi_driver = {
	.probe	= if_spi_probe,
	.remove = __devexit_p(cabrio_spi_remove),
	.driver = {
		.name	= "cabrio_spi",
		.bus	= &spi_bus_type,
		.owner	= THIS_MODULE,
...
static int __init if_spi_init_module(void)
{
	int ret = 0;
	cabrio_dbg_enter(CABRIO_DBG_SPI);
	printk(KERN_INFO "cabrio_spi: Libertas SPI driver\n");
	ret = spi_register_driver(&cabrio_spi_driver);
	cabrio_dbg_leave(CABRIO_DBG_SPI);
	return ret;
}
...
{
	cabrio_dbg_enter(CABRIO_DBG_SPI);
	spi_unregister_driver(&cabrio_spi_driver);
	cabrio_dbg_leave(CABRIO_DBG_SPI);
}
