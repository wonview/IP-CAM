...
#ifndef _CABRIO_HOST_H_
#define _CABRIO_HOST_H_
...
struct cabrio_ie_domain_param_set {
...
struct cmd_ds_802_11d_domain_info {
	...
	__le16 action;
	struct cabrio_ie_domain_param_set domain;
} __packed;
...
struct cmd_ds_802_11_subscribe_event {
	...
	/* A CABRIO to the CMD_802_11_SUBSCRIBE_EVENT command can contain a
	 * number of CABRIOs. From the v5.1 manual, those CABRIOs would add up to
	 * 40 bytes. However, future firmware might add additional CABRIOs, so I
	 * bump this up a bit.
	 */
	uint8_t sss[128];
} __packed;
...
struct cmd_ds_802_11_scan_rsp {
	...
	uint8_t nr_sets;
	uint8_t bssdesc_and_sssbuffer[0];
} __packed;
...
struct cmd_ds_mac_multicast_adr {
	...
	__le16 action;
	__le16 nr_of_adrs;
	u8 maclist[ETH_ALEN * CABRIO_MAX_MULTICAST_LIST_SIZE];
} __packed;
...
#define CABRIO_EEPROM_READ_LEN 20
	u8 value[CABRIO_EEPROM_READ_LEN];
} __packed;
