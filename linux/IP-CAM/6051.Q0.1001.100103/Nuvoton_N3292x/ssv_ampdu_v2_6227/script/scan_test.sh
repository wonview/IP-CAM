...
echo "====================================================================================="
	...
	echo "################################################################################" >> $8
	echo "Unload/Reload test start at   : `date`" >> $8 	
	...
	do
		if ping -W 1 -c 1 $2 >/dev/null; then
			pingok=1
			echo "LOOP[$i] : Connection established in $w seconds. (Passed / Failed / Total = $p / $f / $1)" >> $8		
			break
		else
			pingok=0
		fi
	...
	if [ "$pingok" = 1 ]; then
		pingok=0
    		p=$p+1
		echo "****** Ping Passed!! (Passed / Failed / Total = $p / $f / $1)******"
		...
		pingok=0		
		f=$f+1
		echo "LOOP[$i] : Connection establishing timeout ! (Passed / Failed / Total = $p / $f / $1)" >> $8
		echo "****** Ping Failed!! (Passed / Failed / Total = $p / $f / $1)******"
	...
	./scan_conn.sh $4 $5 off
	sleep 1
	echo "unloading driver"
	./unload.sh
	echo "========================================================="
	echo "Current loop = $i"		
	echo "Current Result : Passed / Failed / Total = $p / $f / $1"		
	echo "========================================================="
	if [ "$i" = "$1" ]; then
		echo "All test loops were finished..."
		echo "Unload/Reload test finished at: `date`" >> $8
		echo "Result : Passed / Failed / Total = $p / $f / $1" >> $8
		echo "################################################################################" >> $8
		echo " " >> $8		
	else		
